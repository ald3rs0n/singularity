...
from Backend.settings import QUARTER, TODAY, YEAR
...
def makeGraph(df,args,tsh):
        ...
        for arg in args:
            ...
            if x == 'MACD':
                ...
                fig.add_trace(vt(df).plotMA({'time':50,'price':'close','type':'EMA','name':'SMA-50'}))
                fig.add_trace(vt(df).plotMA({'time':200,'price':'close','type':'EMA','name':'SMA-200'}))
